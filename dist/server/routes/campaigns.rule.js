...
const campaignsRuleRouter = express_1.Router();
exports.campaignsRuleRouter = campaignsRuleRouter;
campaignsRuleRouter.post("/", (request, response) => {
...
campaignsRuleRouter.post("/search", (request, response) => {
...
campaignsRuleRouter.post("/add", (request, response) => {
...
function getSearchResults(searchTerm, page, totalRecords, merchantBranchId, cb) {
    dbFactory.get()
        ...
        req.query("select count(*) as totalCount from [dbo].[CampaignRule] " + wherequery)
            .then(function (recordset) {
            let totalCount = recordset["recordset"][0].totalCount, query, where;
            ...
            query = dbFactory.getPaginationQuery('Campaign', page, totalRecords, where, " ORDER BY CampaignId");
            console.log("QUery is " + dbFactory.getPaginationQuery('CampaignRule', page, totalRecords, wherequery, " ORDER BY CampaignRuleId"));
            req.query(dbFactory.getPaginationQuery('CampaignRule', page, totalRecords, wherequery, " ORDER BY CampaignRuleId"))
                .then(function (recordset) {
                let data = {
...
function getCampaigns(inputData, merchantBranchId, cb) {
    let page = inputData.pageNumber, totalRecords = inputData.size, searchTerm = inputData.searchTerm, totalPages = inputData.totalPages, sortingField = inputData.sortingField || 'CampaignRuleId', sortingOrder = inputData.sortingOrder || 'desc';
    dbFactory.get()
        .then(function (conn) {
        // Create request instance, passing in connection instance
        ...
        req.query(`select count(*) as totalCount from [dbo].[CampaignRule] ${where}`)
            .then(function (recordset) {
            let totalCount = recordset["recordset"][0].totalCount, query;
            query = dbFactory.getPaginationQuery('CampaignRule', page, totalRecords, where, ` ORDER BY ${sortingField} ${sortingOrder}`);
...
campaignsRuleRouter.post("/delete", (request, response) => {
    let model = request.body;
...
campaignsRuleRouter.post("/update", (request, response) => {
    let model = request.body;
...
//# sourceMappingURL=C:/Users/pytechnovm/Desktop/PYPRIME/Deploy14thNov2017Working26Nov/Deploy14thNov2017Working26Nov/dist/server/routes/campaigns.rule.js.map