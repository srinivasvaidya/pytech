...
transactionsRouter.post("/search", (request, response) => {
    let merchantBranchId = dbFactory.getMerchantIds(request.headers["merchantbranchid"]);
...
function getTransactions(inputData, merchantBranchId, cb) {
    let page = inputData.pageNumber, totalRecords = inputData.size, totalPages = inputData.totalPages, sortingField = inputData.sortingField || 'CustomerInvoiceId', sortingOrder = inputData.sortingOrder || 'desc';
    ...
    dbFactory.get()
        ...
        req.query(`select count(*) as totalCount from [dbo].[CustomerInvoice] where MerchantBranchId in ${merchantBranchId}`)
            ...
            req.query(dbFactory.getPaginationQuery('CustomerInvoice', page, totalRecords, ` WHERE MerchantBranchId in ${merchantBranchId} `, ` ORDER BY ${sortingField} ${sortingOrder}`))
                .then(function (recordset) {
                /*processLastTransactionDate(recordset["recordset"], merchantBranchId, (rs) => {
                    processTotalTransactions(rs, merchantBranchId, (frs) => {
                        let data = {
...
transactionsRouter.post("/add", (request, response) => {
...
//# sourceMappingURL=C:/Users/pytechnovm/Desktop/PYPRIME/Deploy14thNov2017Working26Nov/Deploy14thNov2017Working26Nov/dist/server/routes/transactions.js.map